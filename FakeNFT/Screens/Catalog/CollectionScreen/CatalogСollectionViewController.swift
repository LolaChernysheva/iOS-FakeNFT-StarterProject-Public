...
protocol CatalogСollectionViewControllerProtocol: AnyObject {
    func renderViewData(
        viewData: CatalogCollectionViewData
    )
...
final class CatalogСollectionViewController: UIViewController, NFTCollectionUpdaterDelegate {
    ...
    private func setup() {
        ...
        heightConstraintCV = nftCollection.heightAnchor.constraint(
            equalToConstant: 0
        )
    ...
    private func calculateCollectionHeight(
        ...
        heightConstraintCV.constant = CGFloat(
            numRows
...
extension CatalogСollectionViewController: NFTCollectionCellDelegate {
    func onLikeButtonTapped(
        cell: NFTCollectionCell
    ) {
        guard let nftModel = cell.getNftModel() else {
            return
        }
        presenter.toggleLikeStatus(
            model: nftModel
        )
    }
