...
protocol UserCollectionViewProtocol: AnyObject {
    func updateCollectionList(with items: [Nft])
    func showLoading()
    func hideLoading()
}

final class UserCollectionViewController: UIViewController {
    var presenter: UserCollectionPresenter?
    ...
    func handleStartView(with collectionList: [Nft]?) {
...
extension UserCollectionViewController: UICollectionViewDelegateFlowLayout, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        guard   let item = presenter?.getCollectionList()[indexPath.row],
                let imageURL = item.images.first else {return cell}
        
        cell.setNFTName(with: item.name)
        cell.setNFTImage(with: imageURL.absoluteString)
        cell.setNFTPrice(with: Float(item.price))
        cell.setNFTRating(with: item.rating)
        cell.setNft(with: item)
...
extension UserCollectionViewController: UserCollectionViewProtocol {
    func updateCollectionList(with collectionList: [Nft]) {
            self.nftCollectionView.reloadData()
